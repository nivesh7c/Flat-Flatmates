...
import {
  Button,
  CardContent, IconButton, Typography, CircularProgress
} from "@mui/material";
...
function AddImage({setStep, flatmateDetail, setFlatMateDetail, createFlatmateListing, isEdit=false, propertyId=null}) {
  ...
  const onChange = (imageList, addUpdateIndex) => {
    if (imageList.length <= maxNumber) {
      ...
      const existingImagelength = existingImage.length;
            let tempImage = [];
            imageList.slice(-(imageList.length - existingImagelength)).forEach(item => {
              if(item?.file){
  ...
  useEffect(() => {
    if(isEdit && propertyId){
        getRequest(`seller_dasboard/get_flatmates_properties_images/${propertyId}/`).then(res => {
          if(res?.data?.data){
  ...
  const deleteImage = (index) => {
    ...
    deleteRequest(`seller_dasboard/del_flatmates_properties_images/${imageData.id}/`).then(res => {
                ...
                >
                  {imageList.map((image, index) => (
                    <div key={index}>
                      ...
                      >
                        { !image?.data_url && 
                        <img src={image} alt="recipe thumbnail" height="35px" width="100px"/>
                        }
                        <div
                          style={{
                            display: "flex",
                            ...
                            <div key={index}>
                              <CardContent
                                sx={{
                                  color: "#ffffff",
                                  border: "1px solid #D7D7D7",
                                  backgroundImage: isEdit ? image : `url(${image.data_url})`,
                                  backgroundPosition: "center",
                                  backgroundSize: "cover",
                                  backgroundRepeat: "no-repeat",
                                  minHeight: "35px",
