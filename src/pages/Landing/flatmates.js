...
import vibe from "../../assets/png/vibe.png";
...
function HomeFlatmates() {
  ...
  const getFilter = () => {
    if (localStorage.getItem("FLATMATES_PREFERENCES")) {
      const rent_preference = JSON.parse(
        localStorage.getItem("FLATMATES_PREFERENCES")
      );
      let str = "?";
  ...
  useEffect(() => {
    getRequest(`flatmate/flatmates_list_filter/${getFilter()}`)
      .then((res) => {
        if (res?.data?.results) {
    ...
    <>
      <Layout showCity={true} setRefresh={setIsRefresh} isRefresh={isRefresh}>
        <div style={{ marginTop: "4%", paddingBottom: "50px" }}>
          ...
          >
            <HomeFilter type={"flatmates"} refreshData={setIsRefresh} isRefresh={isRefresh}/>
          </div>
            ...
            <Grid container spacing={2}>
              ...
              <Grid xs={12} sm={6} style={{ display: "grid" }}>
                <div
                  style={{
                    // width: "100%",
                    backgroundImage: `linear-gradient(to bottom,  ${color1} 0%,${color2} 100%), url(${vibe})`,
                    minHeight: "300px",
                    backgroundSize: "cover",
                    backgroundRepeat: "no-repeat",
                    color: "#000",
                ...
                >
                  <Typography sx={{ fontWeight: "600" }}>
                    #Vibecheck with diverse <br />
                    profiles to find the perfect <br />
                    Flatmates
                  </Typography>
              ...
              <Grid md={12}>
                <Typography variant="h6">Shared Apartments</Typography>
              </Grid>
              <Swiper
                id="swiper-color"
                effect={"fade"}
                    ...
                    return (
                      <SwiperSlide>
                        <Grid
                          ...
                          onClick={() => {
                            history.push(
                              `/detail/flatmates/${item?.city}/${item?.id}`
                            );
                          }}
                        >
                          <FlatmateListingCard data={item} />
                        </Grid>
                      </SwiperSlide>
                    );
                  })}
              ...
              <Grid xs={12} sm={6}>
                <div
                  style={{
                    // width: "100%",
                    backgroundImage: `linear-gradient(to right,  ${color4} 0%,${color5} 100%), url(${vibe})`,
                    minHeight: "200px",
                    backgroundSize: "cover",
                    backgroundRepeat: "no-repeat",
                    color: "#000",
