...
import "../../css/flatemateRental.css";
...
function HomeRental() {
  ...
  const getFilter = () => {
    if (localStorage.getItem("RENT_PREFERENCES")) {
      const rent_preference = JSON.parse(
        localStorage.getItem("RENT_PREFERENCES")
      );
      let str = "?";
  ...
  useEffect(() => {
    getRequest(`rent_properties/listing/${getFilter()}`)
      .then((res) => {
        if (res?.data?.results) {
      ...
      .catch((err) => {
        alert.error("Error in fetching rental apartments!!");
      });
  }, []);
    ...
    <>
      <Layout showCity={true} setRefresh={setIsRefresh} isRefresh={isRefresh}>
        <div style={{ marginTop: "4%", paddingBottom: "50px" }}>
          ...
          >
            <HomeFilter type={"rental"} refreshData={setIsRefresh} isRefresh={isRefresh}/>
          </div>
                    ...
                    return (
                      <SwiperSlide>
                        <Grid
                          ...
                          onClick={() => {
                            history.push(
                              `/detail/rental/${item?.city}/${item?.id}`
                            );
                          }}
                        >
                          <RentalListingCard data={item} />
                        </Grid>
                      </SwiperSlide>
                    );
                  })}
